...
Rails (>= 2.3) new support for +I18n+ the solution is a simple helper method.
...
Need a custom title, or need to fill in some placeholders? Just provide a block, in e.g. <tt>contacts/show.html.erb</tt>:

    <h1><%=h page_title { @contact.name } %></h1>
    
A call to <tt>page_title</tt> will now return the contacts name, neat :) if for example the +h1+ does not match the +title+, then
...
Change the application name (and tagline)
-----------------------------------------
